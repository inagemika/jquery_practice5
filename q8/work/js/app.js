...
$(function() {
    ...
    //検索ワードとして入力された値に対応する検索結果を表示
        ...
        /*0より大きい値(検索結果が見つかり表示される状態）の時、もし取得された各項目の0番目の最初の詳細(items)が何もない（null)場合、undefinedとなり何も詳細がない未定の状態となり(void 0)、
        取得された各項目の0番目の最初の詳細(items)があった場合、取得される詳細の個数を表示（検索結果として見つかった0番目の最初の詳細から順に表示（response.length)）*/
            $.each(searchWord[0].items, function(index,data) {
            //検索ワードとして入力された値に対応する検索結果として表示される値の、0番目の値からループ処理で各項目と共通する値を抽出して表示
                let resultTxt = '<li class="lists-item"><div class="list-inner"><p>タイトル：' +
